...
export const getCommentsOfPost=async(req,res)=>{
    try {
        ...
        populate('author','username proflePicture');
        if(!comments){
            return res.status(404).json({
...
export const bookmarkPost = async (req, res) => {
    try {
        ...
        if (isBookmarked) {
            ...
            // Save the post
            await User.findByIdAndUpdate(authorId, { $addToSet: { bookmarks: postId } });
        }

        // After updating, find the user again to get the latest data
        const updatedUser = await User.findById(authorId);

        return res.status(200).json({
            message: isBookmarked ? 'Post removed from bookmark' : 'Post bookmarked',
            success: true,
            bookmarks: updatedUser.bookmarks // ALWAYS return the new bookmarks array
        });
